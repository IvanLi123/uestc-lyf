...
public class VirtualTeller {
  ...
  public int openAccount(String name) {
    ...
    accounts.insert((Keyable) newData);
 ...
 * @throws Exception 
   */
  public void deposit(int acct, int amount) throws Exception {
    AccountData account = findAccount(acct);

    if (account == null) { 
      Exception BadAccountException=new BadAccountException(acct);
		//System.out.println("Error:  Couldn't find account number `" +
      //                   acct + "'");
    	throw BadAccountException;
    } else {
